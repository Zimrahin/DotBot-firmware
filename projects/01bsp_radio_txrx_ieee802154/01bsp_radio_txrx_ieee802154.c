...
static void _radio_callback(uint8_t *packet, uint8_t length, bool crc) {
    ...
    printf("crc %d (%X), (%dB): %s, RSSI: %i\n", crc, NRF_RADIO->RXCRC, length, (char *)packet, db_radio_ieee_802154_rssi());
}
...
int main(void) {
    ...
    db_radio_ieee_802154_set_frequency(RADIO_FREQ);
    db_radio_ieee_802154_rx();

    while (1) {
        db_radio_ieee_802154_disable();
        db_radio_ieee_802154_tx((uint8_t *)packet_tx, sizeof(packet_tx) / sizeof(packet_tx[0]));
